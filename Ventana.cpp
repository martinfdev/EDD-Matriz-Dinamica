...
void Ventana::random(){
    ...
    {
        ran = drand48()*(27)+1;
        Ficha *tmpF = lFichas->del_by_position(ran);
        lFichas->add_endS(tmpF);
        cout<<ran<<"\n";
    } 
}
...
Ventana::Ventana(ListaDoble<string> *diccionario_, BST *usuario_, Config *config_, Lista<Ficha*>*, MatrizDispersa *)
{
...
void Ventana::update_wp2()
{
	...
	mvwprintw(wp2, 4, 1, "Score: ");
	mvwprintw(wp2, 6, (getmaxx(wp2)/2)-(strlen("Letras")/2), "Letras");
	mvwprintw(wp2, 8, (getmaxx(wp2)/2)-(strlen("Player 2")/2), "Player 2");
	mvwprintw(wp2, 10, (getmaxx(wp2)/2)-(strlen("Player 2")/2), "Player 2");
	...
	while (ch = wgetch(menuw))
	{
		...
		case CONTROL_NEXT:
			if (i == 0)
			{
				...
				if (true/*arbolJugadores->getSize() >= 2*/)
				{
					init_windows();
					delwin(menuw);
					return 1;
